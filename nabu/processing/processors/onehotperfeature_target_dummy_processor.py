'''@file onehotperfeature_target_dummy_processor.py
contains the onehotperfeatureTargetDummyProcessor class'''
...
class onehotperfeatureTargetDummyProcessor(processor.Processor):
    '''a processor for audio files, this will compute the targets'''

    def __init__(self, conf, segment_lengths):
        '''onehotperfeatureTargetDummyProcessor constructor

        Args:
            conf: onehotperfeatureTargetDummyProcessor configuration as a dict of strings
        ...
        super(onehotperfeatureTargetDummyProcessor, self).__init__(conf)
	...
	features = np.expand_dims(features, 2)
        ...
        for spk_ind in range(1,self.nrS):
	    features_spk=features+np.random.randn(np.shape(features)[0],np.shape(features)[1],np.shape(features)[2])
	    clean_features = np.append(clean_features,features_spk,2)
