...
outputdir = '/esat/spchtemp/scratch/r0450064/expDir/wsj/DC_WSJ/test/reconstructions/task0/raw_output'
expdir = '/esat/spchtemp/scratch/r0450064/expDir/wsj/DC_WSJ/test'
...
binary_targets_spk1 = np.reshape(binary_targets_spk1,noise_targets_shape[0]*noise_targets_shape[1])
binary_targets_spk2 = binary_targets_complete[:,1::nrS]
binary_targets_spk2 = np.reshape(binary_targets_spk2,noise_targets_shape[0]*noise_targets_shape[1])


emb_vec = np.reshape(emb_vec,[noise_targets_shape[0]*noise_targets_shape[1],20])
...
plt.plot(emb_vec_proj_spk1[:,0],emb_vec_proj_spk1[:,1],'r.')
plt.plot(emb_vec_proj_spk2[:,0],emb_vec_proj_spk2[:,1],'g.')
#ax = fig.add_subplot(111, projection='3d')

#ax.scatter(emb_vec_proj_noise[:,0],emb_vec_proj_noise[:,1],emb_vec_proj_noise[:,2],'b.')
#plt.hold(True)
#ax.scatter(emb_vec_proj_spk1[:,0],emb_vec_proj_spk1[:,1],emb_vec_proj_spk1[:,2],'r.')
#ax.scatter(emb_vec_proj_spk2[:,0],emb_vec_proj_spk2[:,1],emb_vec_proj_spk2[:,2],'g.')
plt.savefig('foo.png')
#plt.show()
