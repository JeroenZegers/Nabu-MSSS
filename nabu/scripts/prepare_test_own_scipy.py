...
def main(_):
	...
	for dupl_ind in range(duplicates_ind_offset, duplicates+duplicates_ind_offset):
		...
		if os.path.isfile(trained_model_checkpoint + '.meta'):
			test_expdir_run = os.path.join(expdir_run, 'test_own_scipy')
			...
			test_expdir_run = os.path.join(expdir_run, 'test_own_scipy_early_%d' % val_step)
			test_model_dir = os.path.join(test_expdir_run, 'model')
		...
		if FLAGS.computing == 'condor':
			...
			for task in tasks:
				...
				subprocess.call([
					...
					'task=%s' % task,
					'script=nabu/scripts/test.py', 'nabu/computing/condor/non_distributed_cpu_own_scipy.job'])
