...
class MultiTaskTrainer():
	...
	nr_tasks = len(self.task_trainers)
		...
		if evaltype != 'None':
			...
			self.validation_loss = tf.reduce_mean(self.val_loss_all_tasks)
			  ...
			  [prev_val_task.assign(self.val_loss_all_tasks[ind]) 
			    for ind,prev_val_task in enumerate(self.previous_validation_all_tasks)] 
                        ...
                        rel_impr = [(self.previous_validation_all_tasks[ind]-self.val_loss_all_tasks[ind])/self.previous_validation_all_tasks[ind] \
			...
			self.update_rel_all_tasks = [tf.assign(rel_val_task, all_rel_imprs[ind]) \
        ...
        with self.graph.as_default():
		    ...
		    ##Validation part
				...
				if val_loss_all_tasks[task_ind] >= prev_best_val_loss_all_tasks[task_ind]:
					...
					if global_step>1000 and np.sum(np.abs(rel_val_loss_all_tasks[task_ind])) < 0.002:
